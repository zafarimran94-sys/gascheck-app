...
/*─── Supabase Mini Client (zero deps, works in artifact) ────*/
...
function Modal({ open, close, title, wide, children }) {
  if (!open) return null;
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={close}><div className="absolute inset-0 bg-black/50 backdrop-blur-sm"/><div className={`relative bg-white rounded-2xl shadow-2xl w-full ${wide?"max-w-2xl":"max-w-md"} max-h-[90vh] overflow-y-auto`} onClick={e=>e.stopPropagation()}><div className="sticky top-0 bg-white rounded-t-2xl border-b px-6 py-4 flex items-center justify-between z-10"><h3 className="text-lg font-bold">{title}</h3><button onClick={close} className="p-1 rounded-full hover:bg-slate-100"><II.No s={20}/></button></div><div className="p-6">{children}</div></div></div>;
}

function Toast({ msg, type, onClose }) {
  useEffect(() => { const t = setTimeout(onClose, 3500); return () => clearTimeout(t); }, []);
  return <div className={`fixed top-4 right-4 z-[100] ${type==="error"?"bg-red-600":type==="success"?"bg-emerald-600":"bg-slate-800"} text-white px-5 py-3 rounded-xl shadow-2xl text-sm font-medium max-w-xs`}>{msg}</div>;
}
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20">
          ...
          <div className="space-y-4">
            <div><label className="block text-xs font-semibold text-slate-300 mb-1.5 uppercase tracking-wider">Email</label><input type="email" className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={em} onChange={e=>setEm(e.target.value)} placeholder="admin@company.com" onKeyDown={e=>e.key==="Enter"&&go()}/></div>
        ...
        <p className="text-center text-slate-500 text-xs mt-6">Create admin user in Supabase Dashboard → Authentication</p>
      </div>
    </div>
  );
}
...
function useData() {
  ...
  const load = useCallback(async () => {
    ...
    if (er.error) console.error(er.error); if (jr.error) console.error(jr.error);
    setLoading(false);
  }, []);
  useEffect(() => { load(); }, [load]);

  const addEmp = async (e) => { const { data, error } = await sb.from("employees").insert(e); if (error) { show(error.message, "error"); return; } if (data?.[0]) setEmps(p => [...p, data[0]]); show("Employee added!", "success"); };
...
/*─── Admin Dashboard ────────────────────────────────────────*/
function Admin({ emps, jobs, onAddEmp, onAddJob, onBulk, onRecon, onViewEmp, onLogout, prof }) {
  const [showE, setShowE] = useState(false); const [showJ, setShowJ] = useState(false);
...
function EmpView({ emp, jobs, onStart, onBack }) {
  ...
  return (
    <div className="min-h-screen" style={{background:"linear-gradient(180deg,#1e40af 0%,#1e40af 180px,#f1f5f9 180px)"}}>
      <div className="px-4 pt-3 pb-6 max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center gap-1.5 text-white/80 hover:text-white text-sm mb-3"><II.Back s={18}/>Back to Admin</button><h1 className="text-2xl font-extrabold text-white">Namaste, {emp.name} 🙏</h1><p className="text-blue-200 text-sm mt-1">आज के काम / Today's Work</p><div className="flex gap-3 mt-4"><div className="bg-white/15 backdrop-blur rounded-xl px-4 py-2.5 text-white"><div className="text-[11px] uppercase opacity-70">Pending</div><div className="text-2xl font-extrabold">{jobs.filter(j=>j.status==="pending"||j.status==="in-progress").length}</div></div><div className="bg-emerald-500/90 rounded-xl px-4 py-2.5 text-white"><div className="text-[11px] uppercase opacity-70">Done</div><div className="text-2xl font-extrabold">{jobs.filter(j=>j.status==="completed").length}</div></div></div></div>
...
export default function App() {
  const [user, setUser] = useState(null); const [prof, setProf] = useState(null); const [authLd, setAuthLd] = useState(true);
  const [view, setView] = useState("admin"); const [selE, setSelE] = useState(""); const [selJ, setSelJ] = useState("");
  const db = useData();

  useEffect(() => { (async () => { const {data:{user:u}}=await sb.auth.getUser(); if(u){setUser(u);const{data:p}=await sb.from("profiles").select("*").eq("id",u.id).single();setProf(p);} setAuthLd(false); })(); }, []);

  const onLogin = async u => { setUser(u); const{data:p}=await sb.from("profiles").select("*").eq("id",u.id).single(); setProf(p); };
  const onLogout = async () => { await sb.auth.signOut(); setUser(null); setProf(null); setView("admin"); };
  const onComplete = async (jobId, upd) => { await db.updJob(jobId, upd); setView("employee"); };
  ...
  if (!user) return <Login onOk={onLogin}/>;

  return (
    <div className="min-h-screen bg-slate-50">
      {db.toast && <Toast msg={db.toast.msg} type={db.toast.type} onClose={()=>db.setToast(null)}/>}
      {db.loading && view==="admin" && <div className="flex justify-center py-20"><II.Spin s={32} className="animate-spin text-blue-500"/></div>}
      {!db.loading && view==="admin" && <Admin emps={db.emps} jobs={db.jobs} onAddEmp={db.addEmp} onAddJob={db.addJob} onBulk={db.addBulk} onRecon={()=>setView("recon")} onViewEmp={id=>{setSelE(id);setView("emp");}} onLogout={onLogout} prof={prof}/>}
