...
function EmpView({emp,jobs,onStart,onBack,onLogout,isDirectLogin}){
  ...
  const JobCard=({j})=>{const isPend=j.status==="pending"||j.status==="in-progress";return(
    <div className={`bg-white rounded-xl border shadow-sm overflow-hidden ${isPend?"border-l-4":"border-slate-200"}`} style={isPend?{borderLeftColor:C.red}:{}}>
      <div className="p-4">
        <div className="flex items-start justify-between mb-3"><SBadge s={j.status}/>{j.consumer_id&&<span className="text-[10px] font-mono bg-slate-100 px-2 py-0.5 rounded text-slate-500">CUID: {j.consumer_id}</span>}</div>
        <div className="space-y-2">
          <div className="flex items-start gap-2"><II.Pin s={16} className="text-slate-400 flex-shrink-0 mt-0.5"/><p className="text-sm font-semibold text-slate-900 leading-snug">{j.address}</p></div>
          {j.customer_name&&<div className="flex items-center gap-2"><II.User s={16} className="text-slate-400 flex-shrink-0"/><p className="text-sm text-slate-700">{j.customer_name}</p></div>}
          {j.customer_phone&&<div className="flex items-center gap-2"><II.Phone s={16} className="text-slate-400 flex-shrink-0"/><a href={`tel:${j.customer_phone}`} className="text-sm text-blue-600 font-medium">{j.customer_phone}</a></div>}
          {j.gas_agency_name&&<div className="text-xs text-slate-400">{j.gas_company_name?`${j.gas_company_name} • `:""}{j.gas_agency_name}</div>}
        </div>
        {isPend&&<button onClick={()=>onStart(j.id)} className="w-full mt-4 py-3.5 text-white rounded-xl font-bold text-sm shadow flex items-center justify-center gap-2 hover:opacity-90 transition" style={{background:"#059669"}}><II.Play s={18}/>काम शुरू / Start Job</button>}
        {j.status==="completed"&&j.payment_amount&&<div className="mt-3 pt-3 border-t border-slate-100 flex justify-between items-center"><span className="text-xs text-slate-500">Payment</span><span className="font-bold text-emerald-600">₹{j.payment_amount} <span className="text-xs font-normal text-slate-400">({(j.payment_type||"").toUpperCase()})</span></span></div>}
      </div>
    </div>
  );};
