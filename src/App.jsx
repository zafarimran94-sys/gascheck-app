...
/*─── Shared ─────────────────────────────────────────────────*/
const statusMap = { pending:"PENDING",completed:"COMPLETED","customer-not-reachable":"NOT REACHABLE","customer-refused":"REFUSED" };
const statusColor = { pending:"bg-amber-100 text-amber-800 border-amber-300",completed:"bg-emerald-100 text-emerald-800 border-emerald-300","customer-not-reachable":"bg-red-100 text-red-800 border-red-300","customer-refused":"bg-rose-100 text-rose-800 border-rose-300" };
const normalizeStatus = s => (["pending","completed","customer-not-reachable","customer-refused"].includes(s) ? s : "pending");
const SBadge = ({ s }) => { const ns=normalizeStatus(s); return <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold tracking-wide border ${statusColor[ns]||"bg-amber-100 text-amber-800 border-amber-300"}`}>{statusMap[ns]||"PENDING"}</span>; };
    ...
    <div className="min-h-screen" style={{background:C.bg}}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5">
        ...
        {tab==="employees"&&<div className="mt-4"><div className="flex justify-between items-center mb-4 flex-wrap gap-2"><h2 className="text-lg font-bold">Field Workers</h2><div className="flex gap-2"><button onClick={()=>setShowAcct(true)} className="flex items-center gap-2 px-4 py-2.5 text-white rounded-lg text-sm font-semibold shadow" style={{background:C.red}}><II.Key s={16}/>Create Login</button><button onClick={()=>setShowE(true)} className="flex items-center gap-2 px-4 py-2.5 text-white rounded-lg text-sm font-semibold shadow" style={{background:C.pri}}><II.Plus s={16}/>Add Employee</button></div></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{emps.map(e=>{const ej=jobs.filter(j=>j.assigned_to===e.id),dn=ej.filter(j=>j.status==="completed").length,pd=ej.filter(j=>normalizeStatus(j.status)==="pending").length;return<div key={e.id} className="bg-white rounded-xl shadow-sm border p-5"><div className="flex items-start justify-between mb-3"><div><h3 className="font-bold">{e.name}</h3><p className="text-sm text-slate-500">{e.phone}</p>{e.area&&<p className="text-xs text-slate-400 mt-0.5">{e.area}</p>}</div><div className="flex items-center gap-1"><div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{background:"#e8edf5"}}><II.User s={16} style={{color:C.pri}}/></div><button onClick={()=>setDelE(e.id)} className="p-1.5 rounded hover:bg-red-50"><II.Trash s={15} className="text-slate-300 hover:text-red-500"/></button></div></div>
        ...
        {tab==="audit"&&(()=>{
          ...
          const auditJobs=jobs.filter(j=>{
            ...
            if(auditSt&&normalizeStatus(j.status)!==auditSt)return false;
                        ...
                        return(
                          <tr key={j.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                            ...
                            <td className="px-4 py-3">
                              {normalizeStatus(j.status)==="pending"
                              ...
                              {normalizeStatus(j.status)!=="pending"&&hasGps&&<div className="text-[9px] text-slate-400 font-mono mt-0.5">{(+j.gps_lat).toFixed(4)}, {(+j.gps_lng).toFixed(4)}</div>}
                            </td>
...
function Recon({emps,jobs,onBack}){
  ...
  const inRangeNorm=inRange.map(j=>({...j,status:normalizeStatus(j.status)}));
  ...
  const es=emps.map(e=>{
    ...
    return{e,tot:ej.length,done:ec.length,pend:ej.filter(j=>normalizeStatus(j.status)==="pending").length,nr:ej.filter(j=>j.status==="customer-not-reachable").length,rf:ej.filter(j=>j.status==="customer-refused").length,cash,upi,already,hoses,rate,rev:cash+upi+already};
